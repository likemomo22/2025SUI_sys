...
namespace Mediapipe
{
    public static class Packet
    {
      ...
      public static Packet<List<bool>> CreateBoolVectorAt(bool[] value, long timestampMicrosec)
        {
            UnsafeNativeMethods
                .mp__MakeBoolVectorPacket_At__Pb_i_ll(value, value.Length, timestampMicrosec, out var ptr).Assert();
      ...
      public static Packet<float[]> CreateFloatArrayAt(float[] value, long timestampMicrosec)
        {
            UnsafeNativeMethods
                .mp__MakeFloatArrayPacket_At__Pf_i_ll(value, value.Length, timestampMicrosec, out var ptr).Assert();
      ...
      public static Packet<List<float>> CreateFloatVectorAt(float[] value, long timestampMicrosec)
        {
            UnsafeNativeMethods
                .mp__MakeFloatVectorPacket_At__Pf_i_ll(value, value.Length, timestampMicrosec, out var ptr).Assert();
      ...
      public static Packet<GpuBuffer> CreateGpuBufferAt(GpuBuffer value, long timestampMicrosec)
        {
            UnsafeNativeMethods.mp__MakeGpuBufferPacket_At__Rgb_ll(value.mpPtr, timestampMicrosec, out var ptr)
                .Assert();
            value.Dispose(); // respect move semantics
      ...
      public static Packet<ImageFrame> CreateImageFrameAt(ImageFrame value, long timestampMicrosec)
        {
            UnsafeNativeMethods.mp__MakeImageFramePacket_At__Pif_ll(value.mpPtr, timestampMicrosec, out var ptr)
                .Assert();
            value.Dispose(); // respect move semantics
      ...
      public static Packet<Matrix> CreateColMajorMatrixAt(float[] data, int row, int col, long timestampMicrosec)
        {
            UnsafeNativeMethods
                .mp__MakeColMajorMatrixPacket_At__Pf_i_i_ll(data, row, col, timestampMicrosec, out var ptr).Assert();
      ...
      public static Packet<TMessage> CreateProto<TMessage>(TMessage value) where TMessage : IMessage<TMessage>
        {
            unsafe
            {
                ...
                UnsafeNativeMethods
                    .mp__PacketFromDynamicProto__PKc_PKc_i(value.Descriptor.FullName, arr, size, out var statusPtr,
                        out var ptr).Assert();
            ...
            where TMessage : IMessage<TMessage>
        {
            unsafe
            {
                ...
                UnsafeNativeMethods.mp__PacketFromDynamicProto_At__PKc_PKc_i_ll(value.Descriptor.FullName, arr, size,
                    timestampMicrosec, out var statusPtr, out var ptr).Assert();
                Status.UnsafeAssertOk(statusPtr);
      ...
      public static Packet<string> CreateStringAt(byte[] value, long timestampMicrosec)
        {
            UnsafeNativeMethods
                .mp__MakeStringPacket_At__PKc_i_ll(value, value?.Length ?? 0, timestampMicrosec, out var ptr).Assert();
    ...
    public class Packet<TValue> : MpResourceHandle
    {
        ...
        /// </param>
        public static Packet<TValue> CreateForReference(IntPtr ptr)
        {
