...
namespace Mediapipe
{
    public class ImageFrame : MpResourceHandle
    {
        ...
        public ImageFrame(ImageFormat.Types.Format format, int width, int height, int widthStep, IntPtr pixelData,
            Deleter deleter)
        {
            UnsafeNativeMethods
                .mp_ImageFrame__ui_i_i_i_Pui8_PF(format, width, height, widthStep, pixelData, deleter, out var ptr)
                .Assert();
            this.ptr = ptr;
        }

        public unsafe ImageFrame(ImageFormat.Types.Format format, int width, int height, int widthStep,
            NativeArray<byte> pixelData, Deleter deleter)
            : this(format, width, height, widthStep, (IntPtr)pixelData.GetUnsafeReadOnlyPtr(), deleter)
        {
        }
        ...
        ///     </list>
        /// </remarks>
        public ImageFrame(ImageFormat.Types.Format format, int width, int height, int widthStep,
        ...
        public ImageFrame(ImageFormat.Types.Format format, Texture2D texture) :
            this(format, texture.width, texture.height, format.NumberOfChannels() * texture.width,
        ...
        [MonoPInvokeCallback(typeof(Deleter))]
