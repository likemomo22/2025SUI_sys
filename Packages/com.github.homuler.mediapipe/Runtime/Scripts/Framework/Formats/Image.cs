...
namespace Mediapipe
{
    public class Image : MpResourceHandle
    {
        ...
        public Image(ImageFormat.Types.Format format, int width, int height, int widthStep, IntPtr pixelData,
            ImageFrame.Deleter deleter)
        {
            UnsafeNativeMethods
                .mp_Image__ui_i_i_i_Pui8_PF(format, width, height, widthStep, pixelData, deleter, out var ptr).Assert();
            this.ptr = ptr;
        }

        public unsafe Image(ImageFormat.Types.Format format, int width, int height, int widthStep,
            NativeArray<byte> pixelData, ImageFrame.Deleter deleter)
