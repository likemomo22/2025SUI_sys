...
namespace Mediapipe
{
    public class CalculatorGraph : MpResourceHandle
    {
        ...
        public void Initialize(CalculatorGraphConfig config)
        {
            ...
            UnsafeNativeMethods.mp_CalculatorGraph__Initialize__PKc_i(mpPtr, bytes, bytes.Length, out var statusPtr)
                .Assert();
            ...
            bool observeTimestampBounds = false)
        {
            UnsafeNativeMethods.mp_CalculatorGraph__ObserveOutputStream__PKc_PF_b(mpPtr, streamName, streamId,
                nativePacketCallback, observeTimestampBounds, out var statusPtr).Assert();

            GC.KeepAlive(this);
            AssertStatusOk(statusPtr);
        }

        public void ObserveOutputStream<T>(string streamName, PacketCallback<T> packetCallback,
            bool observeTimestampBounds, out GCHandle callbackHandle)
        {
        ...
        {
            UnsafeNativeMethods.mp_CalculatorGraph__AddOutputStreamPoller__PKc_b(mpPtr, streamName,
                observeTimestampBounds, out var statusPtr, out var pollerPtr).Assert();
        ...
        {
            UnsafeNativeMethods.mp_CalculatorGraph__CloseInputStream__PKc(mpPtr, streamName, out var statusPtr)
                .Assert();
        ...
        {
            UnsafeNativeMethods
                .mp_CalculatorGraph__SetGpuResources__SPgpu(mpPtr, gpuResources.sharedPtr, out var statusPtr).Assert();
