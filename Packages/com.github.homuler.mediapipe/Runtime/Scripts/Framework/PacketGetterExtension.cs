    ...
    public static class PacketGetterExtension
    {
      ...
      public static bool Get(this Packet<bool> packet)
        ...
        public static void Get(this Packet<List<Image>> packet, List<Image> value)
        {
            ...
            foreach (var image in value) image.Dispose();
            value.Clear();

            foreach (var imagePtr in imageArray.AsReadOnlySpan()) value.Add(new Image(imagePtr));
        }
        ...
        [Obsolete("Use Get instead")]
        public static List<T> GetProtoList<T>(this Packet<List<T>> packet, MessageParser<T> parser)
            where T : IMessage<T>
        {
            return Get(packet, parser);
        }
        ...
        ///     If the <see cref="Packet" /> doesn't contain a proto message list.
        /// </exception>
        public static void Get<T>(this Packet<List<T>> packet, MessageParser<T> parser, List<T> value)
            where T : IMessage<T>
        {
            UnsafeNativeMethods.mp_Packet__GetVectorOfProtoMessageLite(packet.mpPtr, out var serializedProtoVector)
                .Assert();
            GC.KeepAlive(packet);
        ...
        public static int WriteProtoListTo<T>(this Packet<List<T>> packet, MessageParser<T> parser, List<T> value)
            where T : IMessage<T>
        {
            return WriteTo(packet, parser, value);
        }
