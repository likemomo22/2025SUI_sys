...
namespace Mediapipe.Unity
{
    public class AssetBundleResourceManager : IResourceManager
    {
        ...
        IEnumerator IResourceManager.PrepareAssetAsync(string name, string uniqueKey, bool overwriteDestination)
        {
            ...
            if (assetBundle == null) yield return LoadAssetBundleAsync();

            var assetLoadReq = assetBundle.LoadAssetAsync<TextAsset>(name);
            yield return assetLoadReq;

            if (assetLoadReq.asset == null) throw new IOException($"Failed to load {name} from {assetBundle.name}");
        ...
        {
            if (Directory.Exists(_CachePathRoot)) Directory.Delete(_CachePathRoot, true);
        }
        ...
        public IEnumerator LoadAssetBundleAsync()
        {
            ...
            if (_assetBundleReq.assetBundle == null) throw new IOException($"Failed to load {_AssetBundlePath}");
        }
