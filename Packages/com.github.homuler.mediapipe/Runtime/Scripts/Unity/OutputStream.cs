...
namespace Mediapipe.Unity
{
    public sealed class OutputStream<T> : IDisposable
    {
        ...
        private static readonly GlobalInstanceTable<int, OutputStream<T>> _InstanceTable = new(20);

        private readonly int _id;

        private readonly ReaderWriterLockSlim _waitTaskLock = new();
        ...
        /// </summary>
        /// <remarks>
        ///     When <paramref name="observeTimestampBounds" /> is set to <see langword="false" />, while there's not output
        ...
        /// </summary>
        /// <remarks>
        ///     When <see cref="observeTimestampBounds" /> is set to <see langword="true" />, <see cref="callback" /> can be
        ///     invoked with an empty packet.
        ...
        public void AddListener(EventHandler<OutputEventArgs> callback, long emptyPacketThresholdMicrosecond = 0)
        {
            ...
            if (OnReceived == null)
                _calculatorGraph.ObserveOutputStream(streamName, _id, InvokeIfOutputStreamFound,
            ...
            OnReceived += (sender, eventArgs) =>
            {
                ...
                if (eventArgs.packet == null && eventArgs.timestampMicrosecond - stream._lastTimestampMicrosec <=
                    emptyPacketThresholdMicrosecond) return;
        ...
        /// </summary>
        /// <remarks>
        ///     Only if <see cref="observeTimestampBounds" /> is set to true, MediaPipe outputs an empty packet when no output is
        ///     present.
        /// </remarks>
        ...
        private Task<NextResult> WaitNextInternal()
        {
            ...
            try
            {
                if (_waitTask != null) return _waitTask;
        ...
        private Task<NextResult> StartWaitNextTask()
        {
            if (_poller == null)
                throw new InvalidOperationException(
                    "OutputStreamPoller is not initialized. Call StartPolling before running the CalculatorGraph");
            ...
            return Task<NextResult>.Factory.StartNew(state =>
            {
                ...
                {
                    if (packet.IsEmpty()) return new NextResult(null, true);
        ...
        [MonoPInvokeCallback(typeof(CalculatorGraph.NativePacketCallback))]
        private static StatusArgs InvokeIfOutputStreamFound(IntPtr graphPtr, int streamId, IntPtr packetPtr)
        {
            try
            {
                ...
                if (outputStream._calculatorGraph.mpPtr != graphPtr)
                    return StatusArgs.InvalidArgument(
                        "OutputStream is found, but is not linked to the specified CalclatorGraph");
        ...
        public readonly struct OutputEventArgs
        {
          ...
          ///     As long as it's not <see langword="null" />, it's guaranteed that <see cref="Packet.IsEmpty" /> is
          ///     <see langword="false" />.
          /// </summary>
