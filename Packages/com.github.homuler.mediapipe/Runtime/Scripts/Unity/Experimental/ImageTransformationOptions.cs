...
namespace Mediapipe.Unity.Experimental
{
    public readonly struct ImageTransformationOptions
    {
        ...
        public static ImageTransformationOptions Build(bool shouldFlipHorizontally, bool isVerticallyFlipped,
            RotationAngle rotation)
        {
            var isInverted = ImageCoordinate.IsInverted(rotation);
            var flipHorizontally = !isInverted && shouldFlipHorizontally;
            var flipVertically = !shouldFlipHorizontally ? !isVerticallyFlipped :
                isInverted ? isVerticallyFlipped : !isVerticallyFlipped;
