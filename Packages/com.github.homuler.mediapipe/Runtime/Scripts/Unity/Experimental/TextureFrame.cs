...
#pragma warning restore IDE0065
    ...
    public class TextureFrame : IDisposable
    {
        ...
        private static readonly GlobalInstanceTable<Guid, TextureFrame> _InstanceTable = new(MaxTotalCount);
...
#pragma warning restore IDE1006
        ...
        public TextureFrame(int width, int height, TextureFormat format) : this(new Texture2D(width, height, format,
            false))
        {
        }
        ...
        ///     If you need to read pixel data on CPU, use <see cref="ReadTextureAsync" /> or <see cref="ReadTextureOnCPU" />
        ///     instead.
        /// </remarks>
        ...
        public void ReadTextureOnCPU(Texture src, bool flipHorizontally = false, bool flipVertically = false)
        {
            ...
            var rect = new UnityEngine.Rect(0, 0, Mathf.Min(_tmpRenderTexture.width, _texture.width),
                Mathf.Min(_tmpRenderTexture.height, _texture.height));
        ...
        ///     Avoid calling another method before the returned <see cref="AsyncGPUReadbackRequest.done" /> becomes
        ///     <see langword="true" />.
        /// </remarks>
        /// <param name="flipHorizontally">If <see langword="true" />, it will be copied horizontally flipped.</param>
        /// <param name="flipVertically">If <see langword="true" />, it will be copied vertically flipped.</param>
        public AsyncGPUReadbackRequest ReadTextureAsync(Texture src, bool flipHorizontally = false,
        ...
        {
            return new ImageFrame(imageFormat, _texture);
        }

        public Image BuildCPUImage()
        {
            return new Image(imageFormat, _texture);
        }

        [Obsolete("Use BuildGPUImage")]
        public Image BuildGpuImage(GlContext glContext)
        {
            return BuildGPUImage(glContext);
        }
...
#if UNITY_EDITOR_LINUX || UNITY_STANDALONE_LINUX || UNITY_ANDROID
      var glTextureBuffer =
 new GlTextureBuffer(GetTextureName(), width, height, gpuBufferformat, OnReleaseTextureFrame, glContext);
      return new GpuBuffer(glTextureBuffer);
        ...
        {
            if (_glSyncToken == null) return;
            _glSyncToken.Wait();
            _glSyncToken.Dispose();
            _glSyncToken = null;
        }

        [MonoPInvokeCallback(typeof(GlTextureBuffer.DeletionCallback))]
        ...
        /// </summary>
        /// <remarks>
        ///     If the instance whose id is <paramref name="ownerId" /> owns <paramref name="name" /> now, it still removes
        ///     <paramref name="name" />.
        /// </remarks>
        ...
        {
            if (_nativeTexturePtr == IntPtr.Zero) return true;

            var currentName = GetTextureName();
            if (!_NameTable.Remove(currentName)) return false;
