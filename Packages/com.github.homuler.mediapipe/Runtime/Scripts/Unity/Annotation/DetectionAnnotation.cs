...
#pragma warning restore IDE0065
    ...
    {
        [FormerlySerializedAs("_locationDataAnnotation")] [SerializeField]
        private RectangleAnnotation _boundingBoxAnnotation;

        [FormerlySerializedAs("_relativeKeypointsAnnotation")] [SerializeField]
        private PointListAnnotation _keypointsAnnotation;
        ...
        ///     This will affect the rectangle's color. For example, if the score is below the threshold, the rectangle will be
        ///     transparent.
        ...
        public void Draw(mptcc.Detection target, Vector2Int imageSize, float threshold = 0.0f)
        {
            if (ActivateFor(target))
            {
                ...
                var rectVertices = GetScreenRect()
                    .GetRectVertices(target.boundingBox, imageSize, rotationAngle, isMirrored);
                _boundingBoxAnnotation.SetColor(GetColor(score, Mathf.Clamp(threshold, 0.0f, 1.0f)));
                _boundingBoxAnnotation.Draw(rectVertices);
