...
namespace Mediapipe.Unity
{
    public sealed class HolisticLandmarkListAnnotation : HierarchicalAnnotation
    {
        ...
        public void Draw(IReadOnlyList<NormalizedLandmark> faceLandmarks,
            IReadOnlyList<NormalizedLandmark> poseLandmarks,
            IReadOnlyList<NormalizedLandmark> leftHandLandmarks, IReadOnlyList<NormalizedLandmark> rightHandLandmarks,
            bool visualizeZ = false, int circleVertices = 128)
        {
            var mask = PoseLandmarkListAnnotation.BodyParts.All;
            if (faceLandmarks != null) mask ^= PoseLandmarkListAnnotation.BodyParts.Face;
            if (leftHandLandmarks != null) mask ^= PoseLandmarkListAnnotation.BodyParts.LeftHand;
            if (rightHandLandmarks != null) mask ^= PoseLandmarkListAnnotation.BodyParts.RightHand;
        ...
        private void RedrawWristJoints()
        {
            if (_connectionListAnnotation[0].isEmpty)
                ...
                _connectionListAnnotation[0]
                    .Draw(new Connection(_poseLandmarkListAnnotation[13], _leftHandLandmarkListAnnotation[0]));
            if (_connectionListAnnotation[1].isEmpty)
                // connect right elbow and wrist
                _connectionListAnnotation[1]
                    .Draw(new Connection(_poseLandmarkListAnnotation[14], _rightHandLandmarkListAnnotation[0]));
