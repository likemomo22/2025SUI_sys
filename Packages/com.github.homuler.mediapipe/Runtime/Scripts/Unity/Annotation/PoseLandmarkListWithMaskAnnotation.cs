...
#pragma warning restore IDE0065
    ...
    public sealed class PoseLandmarkListWithMaskAnnotation : HierarchicalAnnotation
    {
        ...
        {
            _maskOverlayAnnotation.Init(screen, width, height);
        }

        public void SetLeftLandmarkColor(Color leftLandmarkColor)
        {
            _poseLandmarkListAnnotation.SetLeftLandmarkColor(leftLandmarkColor);
        }

        public void SetRightLandmarkColor(Color rightLandmarkColor)
        {
            _poseLandmarkListAnnotation.SetRightLandmarkColor(rightLandmarkColor);
        }

        public void SetLandmarkRadius(float landmarkRadius)
        {
            _poseLandmarkListAnnotation.SetLandmarkRadius(landmarkRadius);
        }

        public void SetConnectionColor(Color connectionColor)
        {
            _poseLandmarkListAnnotation.SetConnectionColor(connectionColor);
        }

        public void SetConnectionWidth(float connectionWidth)
        {
            _poseLandmarkListAnnotation.SetConnectionWidth(connectionWidth);
        }

        public void SetMaskTexture(Texture2D maskTexture, Color color)
        {
            _maskOverlayAnnotation.SetMaskTexture(maskTexture, color);
        }

        public void SetMaskThreshold(float threshold)
        {
            _maskOverlayAnnotation.SetThreshold(threshold);
        }

        public void ReadMask(Image segmentationMask, bool isMirrored = false)
        {
            _maskOverlayAnnotation.Read(segmentationMask, isMirrored);
        }
