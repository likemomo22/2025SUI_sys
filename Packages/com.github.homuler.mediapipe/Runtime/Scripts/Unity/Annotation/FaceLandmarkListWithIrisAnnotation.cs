...
#pragma warning restore IDE0065
    ...
    public sealed class FaceLandmarkListWithIrisAnnotation : HierarchicalAnnotation
    {
        ...
        {
            if (ActivateFor(target)) Draw(target.Landmark, visualizeZ, circleVertices);
        }
        ...
        {
            if (ActivateFor(target)) Draw(target.landmarks, visualizeZ, circleVertices);
        }
        ...
        private void DrawLeftIrisLandmarkList(IReadOnlyList<NormalizedLandmark> target, bool visualizeZ = false,
            int circleVertices = 128)
        {
        ...
        private void DrawLeftIrisLandmarkList(IReadOnlyList<mptcc.NormalizedLandmark> target, bool visualizeZ = false,
            int circleVertices = 128)
        {
            // does not deactivate if the target is null as long as face landmarks are present.
            _leftIrisLandmarkListAnnotation.Draw(target, visualizeZ, circleVertices);
        ...
        private void DrawRightIrisLandmarkList(IReadOnlyList<NormalizedLandmark> target, bool visualizeZ = false,
            int circleVertices = 128)
        {
            // does not deactivate if the target is null as long as face landmarks are present.
        ...
        private void DrawRightIrisLandmarkList(IReadOnlyList<mptcc.NormalizedLandmark> target, bool visualizeZ = false,
            int circleVertices = 128)
        {
            // does not deactivate if the target is null as long as face landmarks are present.
            _rightIrisLandmarkListAnnotation.Draw(target, visualizeZ, circleVertices);
        ...
        private static (IReadOnlyList<T>, IReadOnlyList<T>, IReadOnlyList<T>) PartitionLandmarkList<T>(
            IReadOnlyList<T> landmarks)
        {
            if (landmarks == null) return (null, null, null);
            ...
            if (faceLandmarks.Count < _FaceLandmarkCount) return (null, null, null);
            ...
            if (leftIrisLandmarks.Count < _IrisLandmarkCount) return (faceLandmarks, null, null);
            ...
            return rightIrisLandmarks.Count < _IrisLandmarkCount
                ? (faceLandmarks, leftIrisLandmarks, null)
                : (faceLandmarks, leftIrisLandmarks, rightIrisLandmarks);
        }
    }
}