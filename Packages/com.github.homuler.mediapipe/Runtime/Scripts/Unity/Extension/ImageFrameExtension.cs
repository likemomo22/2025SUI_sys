...
namespace Mediapipe.Unity
{
    public static class ImageFrameExtension
    {
      ...
      /// </param>
      public static bool TryReadChannel(this ImageFrame imageFrame, int channelNumber, byte[] channelData,
            bool isHorizontallyFlipped = false, bool isVerticallyFlipped = false)
        {
            return TryReadChannel(imageFrame.Format(), imageFrame.Width(), imageFrame.Height(), imageFrame.WidthStep(),
                imageFrame.ByteDepth(),
      ...
      /// </param>
      public static bool TryReadChannel(this Image image, int channelNumber, byte[] channelData,
            bool isHorizontallyFlipped = false, bool isVerticallyFlipped = false)
        {
        ...
        private static bool TryReadChannel(ImageFormat.Types.Format format, int width, int height, int widthStep,
            int byteDepth,
            int channelNumber, IntPtr pixelData, byte[] channelData, bool isHorizontallyFlipped,
            bool isVerticallyFlipped)
        {
            var channelCount = format.NumberOfChannels();
            if (!IsChannelNumberValid(channelCount, channelNumber)) return false;
...
#pragma warning disable IDE0010
            switch (format)
            {
                ...
                case ImageFormat.Types.Format.Lab8:
                    return TryReadChannel(width, height, widthStep, byteDepth, channelCount, channelNumber, pixelData,
                        channelData, isHorizontallyFlipped, isVerticallyFlipped);
                default:
...
#pragma warning restore IDE0010
        ...
        ///     Set <c>true</c> if the <paramref name="imageFrame" /> is flipped vertically.
        /// </param>
        public static bool TryReadChannel(this ImageFrame imageFrame, int channelNumber, ushort[] channelData,
            bool isHorizontallyFlipped = false, bool isVerticallyFlipped = false)
        {
            return TryReadChannel(imageFrame.Format(), imageFrame.Width(), imageFrame.Height(), imageFrame.WidthStep(),
                imageFrame.ByteDepth(),
        ...
        ///     Set <c>true</c> if the <paramref name="image" /> is flipped vertically.
        /// </param>
        public static bool TryReadChannel(this Image image, int channelNumber, ushort[] channelData,
            bool isHorizontallyFlipped = false, bool isVerticallyFlipped = false)
        {
            var format = image.ImageFormat();
            using (var pixelWriteLock = new PixelWriteLock(image))
        ...
        private static bool TryReadChannel(ImageFormat.Types.Format format, int width, int height, int widthStep,
            int byteDepth,
            int channelNumber, IntPtr pixelData, ushort[] channelData, bool isHorizontallyFlipped,
            bool isVerticallyFlipped)
        {
            var channelCount = format.NumberOfChannels();
            if (!IsChannelNumberValid(channelCount, channelNumber)) return false;
...
#pragma warning restore IDE0010
        ...
        ///     Set <c>true</c> if the <paramref name="imageFrame" /> is flipped vertically.
        /// </param>
        public static bool TryReadChannel(this ImageFrame imageFrame, int channelNumber, float[] channelData,
            bool isHorizontallyFlipped = false, bool isVerticallyFlipped = false)
        {
            return TryReadChannel(imageFrame.Format(), imageFrame.Width(), imageFrame.Height(), imageFrame.WidthStep(),
                imageFrame.ByteDepth(),
        ...
        ///     Set <c>true</c> if the <paramref name="image" /> is flipped vertically.
        /// </param>
        public static bool TryReadChannel(this Image image, int channelNumber, float[] channelData,
            bool isHorizontallyFlipped = false, bool isVerticallyFlipped = false)
        {
            var format = image.ImageFormat();
            using (var pixelWriteLock = new PixelWriteLock(image))
        ...
        private static bool TryReadChannel(ImageFormat.Types.Format format, int width, int height, int widthStep,
            int byteDepth,
            int channelNumber, IntPtr pixelData, float[] channelData, bool isHorizontallyFlipped,
            bool isVerticallyFlipped)
        {
            var channelCount = format.NumberOfChannels();
            if (!IsChannelNumberValid(channelCount, channelNumber)) return false;
...
#pragma warning restore IDE0010
        ...
        ///     Set <c>true</c> if the <paramref name="imageFrame" /> is flipped vertically.
        /// </param>
        public static bool TryReadChannelNormalized(this ImageFrame imageFrame, int channelNumber,
            float[] normalizedChannelData, bool isHorizontallyFlipped = false, bool isVerticallyFlipped = false)
        {
            return TryReadChannelNormalized(imageFrame.Format(), imageFrame.Width(), imageFrame.Height(),
                imageFrame.WidthStep(), imageFrame.ByteDepth(),
                channelNumber, imageFrame.MutablePixelData(), normalizedChannelData, isHorizontallyFlipped,
                isVerticallyFlipped);
        }
        ...
        ///     Set <c>true</c> if the <paramref name="image" /> is flipped vertically.
        /// </param>
        public static bool TryReadChannelNormalized(this Image image, int channelNumber, float[] normalizedChannelData,
            bool isHorizontallyFlipped = false, bool isVerticallyFlipped = false)
        {
            var format = image.ImageFormat();
            using (var pixelWriteLock = new PixelWriteLock(image))
            {
                return TryReadChannelNormalized(format, image.Width(), image.Height(), image.Step(), format.ByteDepth(),
                    channelNumber, pixelWriteLock.Pixels(), normalizedChannelData, isHorizontallyFlipped,
                    isVerticallyFlipped);
            }
        }

        private static bool TryReadChannelNormalized(ImageFormat.Types.Format format, int width, int height,
            int widthStep, int byteDepth,
            int channelNumber, IntPtr pixelData, float[] normalizedChannelData, bool isHorizontallyFlipped,
            bool isVerticallyFlipped)
        {
            var channelCount = format.NumberOfChannels();
            if (!IsChannelNumberValid(channelCount, channelNumber)) return false;
...
#pragma warning disable IDE0010
            switch (format)
            {
                ...
                case ImageFormat.Types.Format.Gray8:
                case ImageFormat.Types.Format.Lab8:
                    return TryReadChannel<byte, float>(width, height, widthStep, byteDepth, channelCount, channelNumber,
                        ByteNormalizer, pixelData, normalizedChannelData, isHorizontallyFlipped, isVerticallyFlipped);
                case ImageFormat.Types.Format.Srgb48:
                case ImageFormat.Types.Format.Srgba64:
                case ImageFormat.Types.Format.Gray16:
                    return TryReadChannel<ushort, float>(width, height, widthStep, byteDepth, channelCount,
                        channelNumber, UshortNormalizer, pixelData, normalizedChannelData, isHorizontallyFlipped,
                        isVerticallyFlipped);
                case ImageFormat.Types.Format.Vec32F1:
                case ImageFormat.Types.Format.Vec32F2:
                    return TryReadChannel(width, height, widthStep, byteDepth, channelCount, channelNumber, pixelData,
                        normalizedChannelData, isHorizontallyFlipped, isVerticallyFlipped);
                default:
...
#pragma warning restore IDE0010
        ...
        {
            return TryReadPixelData(imageFrame.Format(), imageFrame.Width(), imageFrame.Height(),
                imageFrame.WidthStep(),
                imageFrame.ByteDepth(), imageFrame.MutablePixelData(), colors);
        }
        ...
        public static bool TryReadPixelData(this Image image, Color32[] colors)
        {
            ...
            using (var pixelWriteLock = new PixelWriteLock(image))
            {
                return TryReadPixelData(format, image.Width(), image.Height(), image.Step(), format.ByteDepth(),
                    pixelWriteLock.Pixels(), colors);
            }
        }

        public static bool TryReadPixelData(ImageFormat.Types.Format format, int width, int height, int widthStep,
            int byteDepth, IntPtr pixelData, Color32[] colors)
        {
#pragma warning disable IDE0010
            switch (format)
            {
...
#pragma warning restore IDE0010
        ...
        private static bool TryReadChannel<T>(int width, int height, int widthStep, int byteDepth, int channelCount,
            int channelNumber,
            IntPtr pixelData, T[] channelData, bool isHorizontallyFlipped, bool isVerticallyFlipped) where T : unmanaged
        {
            ...
            {
                Logger.LogWarning(
                    $"The length of channelData ({channelData.Length}) does not equal {width} * {height} = {length}");
                return false;
            }
        ...
        private static bool TryReadChannel<TSrc, TDst>(
            int width, int height, int widthStep, int byteDepth, int channelCount, int channelNumber,
            ChannelTransformer<TSrc, TDst> transformer,
            IntPtr pixelData, TDst[] channelData, bool isHorizontallyFlipped, bool isVerticallyFlipped)
        ...
        private static bool IsChannelNumberValid(int channelCount, int channelNumber)
        {
            ...
            {
                Logger.LogWarning(
                    $"channelNumber must be <= {channelCount - 1} since there are only {channelCount} channels, but {channelNumber} is given");
                return false;
            }
        ...
        private static bool TryReadSrgb(int width, int height, int widthStep, int byteStep, IntPtr pixelData,
            Color32[] colors)
        {
            var length = width * height;
            ...
            {
                Logger.LogWarning(
                    $"The length of colors ({colors.Length}) does not equal {width} * {height} = {length}");
                return false;
            }
        ...
        private static bool TryReadSrgba(int width, int height, int widthStep, int byteStep, IntPtr pixelData,
            Color32[] colors)
        {
            var length = width * height;
            if (colors.Length != length)
        ...
        private static bool TryReadSbgra(int width, int height, int widthStep, int byteStep, IntPtr pixelData,
            Color32[] colors)
        {
            var length = width * height;
            if (colors.Length != length)
        ...
        private static bool TryReadGray8(int width, int height, int widthStep, int byteStep, IntPtr pixelData,
            Color32[] colors)
        {
            var length = width * height;
            if (colors.Length != length)
        ...
        private static bool TryReadLab8(int width, int height, int widthStep, int byteStep, IntPtr pixelData,
            Color32[] colors)
        {
            var length = width * height;
            if (colors.Length != length)
        ...
        private static bool TryReadSrgb48(int width, int height, int widthStep, int byteStep, IntPtr pixelData,
            Color32[] colors)
        {
            var length = width * height;
            if (colors.Length != length)
        ...
        private static bool TryReadSrgba64(int width, int height, int widthStep, int byteStep, IntPtr pixelData,
            Color32[] colors)
        {
            var length = width * height;
            if (colors.Length != length)
        ...
        private static bool TryReadGray16(int width, int height, int widthStep, int byteStep, IntPtr pixelData,
            Color32[] colors)
        {
            var length = width * height;
            if (colors.Length != length)
        ...
        private static bool TryReadVec32f1(int width, int height, int widthStep, int byteStep, IntPtr pixelData,
            Color32[] colors)
        {
            var length = width * height;
            if (colors.Length != length)
        ...
        private static bool TryReadVec32f2(int width, int height, int widthStep, int byteStep, IntPtr pixelData,
            Color32[] colors)
        {
            var length = width * height;
            if (colors.Length != length)
