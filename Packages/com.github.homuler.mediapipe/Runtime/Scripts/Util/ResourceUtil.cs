...
namespace Mediapipe
{
    public static class ResourceUtil
    {
        ...
        private static readonly Dictionary<string, string> _AssetPathMap = new();
        ...
        [MonoPInvokeCallback(typeof(PathResolver))]
        private static string PathToResourceAsFile(string assetPath)
        {
            Debug.Log($"{assetPath} is requested");
            try
            {
                Logger.LogDebug(_TAG, $"{assetPath} is requested");
                if (TryGetFilePath(assetPath, out var filePath)) return filePath;
        ...
        [MonoPInvokeCallback(typeof(NativeResourceProvider))]
        private static bool GetResourceContents(string path, IntPtr dst)
        {
            Debug.Log($"{path} is requested");
