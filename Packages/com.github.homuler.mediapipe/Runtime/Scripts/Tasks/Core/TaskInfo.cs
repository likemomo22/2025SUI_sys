...
namespace Mediapipe.Tasks.Core
{
    internal class TaskInfo<T> where T : ITaskOptions
    {
        ...
        public CalculatorGraphConfig GenerateGraphConfig(bool enableFlowLimiting = false)
        {
            ...
            flowLimiterOptions.SetExtension(FlowLimiterCalculatorOptions.Extensions.Ext,
                new FlowLimiterCalculatorOptions
                {
                    MaxInFlight = 1,
                    MaxInQueue = 1
                });
        ...
        private static CalculatorGraphConfig.Types.Node BuildConfigNode(string calculator, T taskOptions,
            IEnumerable<string> inputStreams, IEnumerable<string> outputStreams)
        {
