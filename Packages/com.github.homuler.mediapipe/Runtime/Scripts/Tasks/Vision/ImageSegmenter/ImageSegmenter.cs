...
namespace Mediapipe.Tasks.Vision.ImageSegmenter
{
    public sealed class ImageSegmenter : BaseVisionTaskApi
    {
        ...
        private const string _TENSORS_TO_SEGMENTATION_CALCULATOR_NAME =
...
#pragma warning restore IDE0052
        ...
        public static ImageSegmenter CreateFromModelPath(string modelPath, GpuResources gpuResources = null)
        {
            ...
            var options = new ImageSegmenterOptions(baseOptions);
        ...
        public static ImageSegmenter CreateFromOptions(ImageSegmenterOptions options, GpuResources gpuResources = null)
        {
            ...
            var taskInfo = new TaskInfo<ImageSegmenterOptions>(
                _TASK_GRAPH_NAME,
                new List<string>
                {
        ...
        /// </returns>
        public bool TrySegment(Image image, ImageProcessingOptions? imageProcessingOptions,
        ...
        ///     <see langword="true" /> if the segmentation is successful, <see langword="false" /> otherwise.
        /// </returns>
        public bool TrySegmentForVideo(Image image, long timestampMillisec,
            ImageProcessingOptions? imageProcessingOptions, ref ImageSegmenterResult result)
        {
            using var outputPackets = SegmentForVideoInternal(image, timestampMillisec, imageProcessingOptions);
            return TryBuildImageSegmenterResult(outputPackets, ref result);
        }
        ...
        ///     input image.
        public void SegmentAsync(Image image, long timestampMillisec,
            ImageProcessingOptions? imageProcessingOptions = null)
        {
            ConfigureNormalizedRect(_normalizedRect, imageProcessingOptions, image, false);
            var timestampMicrosec = timestampMillisec * _MICRO_SECONDS_PER_MILLISECOND;
        ...
        private List<string> GetLabels()
        {
            ...
            foreach (var node in graphConfig.Node)
                if (node.Name.EndsWith(_TENSORS_TO_SEGMENTATION_CALCULATOR_NAME))
                {
                    ...
                    {
                        foreach (var labelItem in options.LabelItems) labels.Add(labelItem.Value.Name);
                        return labels;
                    }
                }
