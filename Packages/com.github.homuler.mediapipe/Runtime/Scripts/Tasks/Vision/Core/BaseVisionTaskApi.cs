  ...
  public class BaseVisionTaskApi : IDisposable
    {
            ...
            TaskRunner.PacketsCallback packetsCallback)
        {
            ...
            var (callbackId, nativePacketsCallback) = PacketsCallbackTable.Add(packetsCallback);
        ...
        protected void ConfigureNormalizedRect(NormalizedRect target, ImageProcessingOptions? options, Image image,
            bool roiAllowed = true)
        {
            ResetNormalizedRect(target);

            if (!(options is ImageProcessingOptions optionsValue)) return;
