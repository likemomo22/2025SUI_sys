...
namespace Mediapipe.Tests
{
    public class PacketTest
    {
        ...
        private void AssertImage(Image image, int width, int height, ImageFormat.Types.Format format,
            byte[] expectedBytes)
        {
        ...
        private void AssertImageFrame(ImageFrame imageFrame, int width, int height, ImageFormat.Types.Format format,
            byte[] expectedBytes)
        {
        ...
        public void CreateBoolVector_ShouldReturnNewBoolVectorPacket()
        {
            ...
            for (var i = 0; i < value.Length; i++) Assert.AreEqual(value[i], result[i]);
        ...
        {
            var value = new[] { float.MinValue, 0f, float.MaxValue };
