...
namespace Mediapipe.Tests.Tasks.Audio
{
    public class AudioClassifierTest
    {
        ...
        private readonly Lazy<TextAsset> _audioClassifierModel =
            new(() => AssetDatabase.LoadAssetAtPath<TextAsset>(
                $"{_ResourcePath}/yamnet_audio_classifier_with_metadata.bytes"));

        private readonly Lazy<AudioClip> _doorSound =
            new(() => AssetDatabase.LoadAssetAtPath<AudioClip>($"{_TestResourcePath}/prison-cell-door.mp3"));
        ...
        public void TryClassify_ShouldReturnAudioClassifierResult_When_AudioClipIsNotEmpty()
        {
            var options = new AudioClassifierOptions(
                new BaseOptions(modelAssetBuffer: _audioClassifierModel.Value.bytes),
                maxResults: 5);
            ...
            using (var audioClassifier = AudioClassifier.CreateFromOptions(options))
            {
                ...
                var audioClip = new Matrix(sound, 2, sampleRate);
        ...
        public IEnumerator ClassifyAsync_ShouldInvokeTheCallback()
        {
            ...
            using (var audioClassifier = AudioClassifier.CreateFromOptions(options))
            {
                ...
                var audioClip = new Matrix(sound, 2, sampleRate * 3);
        ...
        {
            var options = new AudioClassifierOptions(new BaseOptions());
        ...
        {
            var options =
                new AudioClassifierOptions(new BaseOptions(modelAssetBuffer: _audioClassifierModel.Value.bytes));
        ...
        {
            var options = new AudioClassifierOptions(new BaseOptions(BaseOptions.Delegate.CPU, "unknown_path.bytes"));
...
#pragma warning restore IDE0056
        ...
        public void Classify_ShouldReturnAudioClassifierResult_When_AudioClipIsNotEmpty()
        {
            ...
            using (var audioClassifier = AudioClassifier.CreateFromOptions(options))
            {
                ...
                var audioClip = new Matrix(sound, source.channels, source.samples);
