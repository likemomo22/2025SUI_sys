...
namespace Mediapipe.Tests.Tasks.Vision
{
    public class ObjectDetectorTest
    {
        ...
        private readonly Lazy<TextAsset> _objectDetectorModel =
            new(() => AssetDatabase.LoadAssetAtPath<TextAsset>($"{_ResourcePath}/efficientdet_lite0_float16.bytes"));
        ...
        {
            var options = new ObjectDetectorOptions(new BaseOptions());
        ...
        {
            var options =
                new ObjectDetectorOptions(new BaseOptions(modelAssetBuffer: _objectDetectorModel.Value.bytes));
        ...
        {
            var options = new ObjectDetectorOptions(new BaseOptions(BaseOptions.Delegate.CPU, "unknown_path.bytes"));
        ...
        public IEnumerator Create_returns_ObjectDetector_when_assetModelPath_is_valid()
        {
            ...
            var options =
                new ObjectDetectorOptions(new BaseOptions(BaseOptions.Delegate.CPU,
        ...
        {
            var options = new ObjectDetectorOptions(new BaseOptions(modelAssetBuffer: _objectDetectorModel.Value.bytes),
            ...
            using (var objectDetector = ObjectDetector.CreateFromOptions(options))
            {
                ...
                {
                    var result = objectDetector.Detect(image);
        ...
        public void Detect_ShouldReturnObjectDetectionResult_When_ObjectsAreDetected()
        {
            var options = new ObjectDetectorOptions(
                new BaseOptions(modelAssetBuffer: _objectDetectorModel.Value.bytes),
                maxResults: 10);
        ...
        public void DetectForVideo_ShouldReturnAnEmptyResult_When_ImageIsEmpty()
        {
            ...
            using (var objectDetector = ObjectDetector.CreateFromOptions(options))
            {
                ...
                using (var image = new Image(ImageFormat.Types.Format.Srgba, width, height, width * 4, pixelData))
                {
                    var result = objectDetector.DetectForVideo(image, 1);
                    Assert.IsNull(result.detections);
                }
            }
        }
