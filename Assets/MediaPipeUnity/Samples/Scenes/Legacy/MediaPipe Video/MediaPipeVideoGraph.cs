...
namespace Mediapipe.Unity.Sample.MediaPipeVideo
{
    public class MediaPipeVideoGraph : GraphRunner
    {
        ...
        {
            if (configType != ConfigType.OpenGLES) _outputVideoStream.StartPolling();
        ...
        {
            if (runningMode == RunningMode.Async) throw new ArgumentException("Asynchronous mode is not supported");
            return base.Initialize(runningMode);
        }

        public void SetupOutputPacket(TextureFrame textureFrame, GlContext glContext)
        {
        ...
        public async Task<ImageFrame> WaitNextAsync()
        {
            ...
            _ = TryGetValue(result.packet, out var outputVideo, packet => { return packet.Get(); });
            return outputVideo;
        }
        ...
        protected override void ConfigureCalculatorGraph(CalculatorGraphConfig config)
        {
            ...
            _outputVideoStream = new OutputStream<ImageFrame>(calculatorGraph, _OutputVideoStreamName);
        ...
        private PacketMap BuildSidePacket(ImageSource imageSource)
        {
            ...
            if (configType == ConfigType.OpenGLES) sidePacket.Emplace(_destinationBufferName, _outputGpuBufferPacket);
