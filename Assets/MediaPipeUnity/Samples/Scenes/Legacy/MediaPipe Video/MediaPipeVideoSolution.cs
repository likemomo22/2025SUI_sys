...
namespace Mediapipe.Unity.Sample.MediaPipeVideo
{
    public class MediaPipeVideoSolution : LegacySolutionRunner<MediaPipeVideoGraph>
    {
        ...
        protected override IEnumerator Run()
        {
            ...
            {
                _outputTexture = new Texture2D(imageSource.textureWidth, imageSource.textureHeight,
            ...
            while (true)
            {
                ...
                if (canUseGpuImage)
                    ...
                    textureFrame.ReadTextureOnCPU(imageSource.GetCurrentTexture(), false,
                        imageSource.isVerticallyFlipped);

                graphRunner.AddTextureFrameToInputStream(textureFrame, glContext);

                if (graphRunner.configType == GraphRunner.ConfigType.OpenGLES) continue;
