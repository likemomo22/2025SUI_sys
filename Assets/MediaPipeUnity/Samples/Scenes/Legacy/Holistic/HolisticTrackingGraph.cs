...
namespace Mediapipe.Unity.Sample.Holistic
{
    public readonly struct HolisticTrackingResult
    {
        ...
        public HolisticTrackingResult(Detection poseDetection, NormalizedLandmarkList poseLandmarks,
            NormalizedLandmarkList faceLandmarks, NormalizedLandmarkList leftHandLandmarks,
            NormalizedLandmarkList rightHandLandmarks, LandmarkList poseWorldLandmarks, ImageFrame segmentationMask,
            NormalizedRect poseRoi)
        {
    ...
    public class HolisticTrackingGraph : GraphRunner
    {
        ...
        public async Task<HolisticTrackingResult> WaitNextAsync()
        {
            ...
            _ = TryGetValue(results.Item1.packet, out var poseDetection,
                packet => { return packet.Get(Detection.Parser); });
            _ = TryGetValue(results.Item2.packet, out var poseLandmarks,
                packet => { return packet.Get(NormalizedLandmarkList.Parser); });
            _ = TryGetValue(results.Item3.packet, out var faceLandmarks,
                packet => { return packet.Get(NormalizedLandmarkList.Parser); });
            _ = TryGetValue(results.Item4.packet, out var leftHandLandmarks,
                packet => { return packet.Get(NormalizedLandmarkList.Parser); });
            _ = TryGetValue(results.Item5.packet, out var rightHandLandmarks,
                packet => { return packet.Get(NormalizedLandmarkList.Parser); });
            _ = TryGetValue(results.Item6.packet, out var poseWorldLandmarks,
                packet => { return packet.Get(LandmarkList.Parser); });
            _ = TryGetValue(results.Item7.packet, out var segmentationMask, packet => { return packet.Get(); });
            _ = TryGetValue(results.Item8.packet, out var poseRoi,
                packet => { return packet.Get(NormalizedRect.Parser); });

            return new HolisticTrackingResult(poseDetection, poseLandmarks, faceLandmarks, leftHandLandmarks,
                rightHandLandmarks, poseWorldLandmarks, segmentationMask, poseRoi);
        }
        ...
        {
            _poseDetectionStream = new OutputStream<Detection>(calculatorGraph, _PoseDetectionStreamName);
            _poseLandmarksStream = new OutputStream<NormalizedLandmarkList>(calculatorGraph, _PoseLandmarksStreamName);
            _faceLandmarksStream = new OutputStream<NormalizedLandmarkList>(calculatorGraph, _FaceLandmarksStreamName);
            _leftHandLandmarksStream =
                new OutputStream<NormalizedLandmarkList>(calculatorGraph, _LeftHandLandmarksStreamName);
            _rightHandLandmarksStream =
                new OutputStream<NormalizedLandmarkList>(calculatorGraph, _RightHandLandmarksStreamName);
            _poseWorldLandmarksStream = new OutputStream<LandmarkList>(calculatorGraph, _PoseWorldLandmarksStreamName);
            _segmentationMaskStream = new OutputStream<ImageFrame>(calculatorGraph, _SegmentationMaskStreamName);
            _poseRoiStream = new OutputStream<NormalizedRect>(calculatorGraph, _PoseRoiStreamName);
            ...
            using (var validatedGraphConfig = new ValidatedGraphConfig())
            {
                ...
                {
                    TensorsToDetectionsCalculatorOptions.Extensions.Ext, ThresholdingCalculatorOptions.Extensions.Ext
                };
                ...
                var tensorsToDetectionsCalculators = cannonicalizedConfig.Node
                    .Where(node => poseDetectionCalculatorPattern.Match(node.Name).Success).ToList();

                var poseTrackingCalculatorPattern =
                    new Regex("tensorstoposelandmarksandsegmentation__ThresholdingCalculator$");
                var thresholdingCalculators = cannonicalizedConfig.Node
                    .Where(node => poseTrackingCalculatorPattern.Match(node.Name).Success).ToList();
                ...
                foreach (var calculator in tensorsToDetectionsCalculators)
                    ...
                    {
                        var options =
                            calculator.Options.GetExtension(TensorsToDetectionsCalculatorOptions.Extensions.Ext);
        ...
        private PacketMap BuildSidePacket(ImageSource imageSource)
        {
            ...
            var outputVerticallyFlipped = (!runningMode.IsSynchronous() && imageSource.isVerticallyFlipped) ^
                                          (isInverted && imageSource.isHorizontallyFlipped);
            ...
            Debug.Log(
                $"outtput_rotation = {outputRotation}, output_horizontally_flipped = {outputHorizontallyFlipped}, output_vertically_flipped = {outputVerticallyFlipped}");
