...
namespace Mediapipe.Unity.Sample
{
    public abstract class GraphRunner : MonoBehaviour
    {
        ...
        [SerializeField] private long _timeoutMicrosec;
...
#pragma warning restore IDE1006
...
#endif
                    if (_gpuConfig != null) return ConfigType.GPU;
                }
        ...
        {
            if (!result.ok) throw new Exception("Failed to get the next packet");
        }
        ...
        protected void AssertResult<T1, T2, T3>(
            (OutputStream<T1>.NextResult, OutputStream<T2>.NextResult, OutputStream<T3>.NextResult) result)
        {
            AssertResult(result.Item1);
            AssertResult(result.Item2);
        ...
        protected void AssertResult<T1, T2, T3, T4>(
            (OutputStream<T1>.NextResult, OutputStream<T2>.NextResult, OutputStream<T3>.NextResult,
                OutputStream<T4>.NextResult) result)
        {
            AssertResult(result.Item1);
            AssertResult(result.Item2);
            AssertResult(result.Item3);
        ...
        protected async Task<(T1, T2, T3, T4)> WhenAll<T1, T2, T3, T4>(Task<T1> task1, Task<T2> task2, Task<T3> task3,
            Task<T4> task4)
        {
        ...
        protected async Task<(T1, T2, T3, T4, T5)> WhenAll<T1, T2, T3, T4, T5>(Task<T1> task1, Task<T2> task2,
            Task<T3> task3, Task<T4> task4, Task<T5> task5)
        {
            await Task.WhenAll(task1, task2, task3, task4, task5);
            return (task1.Result, task2.Result, task3.Result, task4.Result, task5.Result);
        }

        protected async Task<(T1, T2, T3, T4, T5, T6)> WhenAll<T1, T2, T3, T4, T5, T6>(Task<T1> task1, Task<T2> task2,
            Task<T3> task3, Task<T4> task4, Task<T5> task5, Task<T6> task6)
        {
            await Task.WhenAll(task1, task2, task3, task4, task5, task6);
            return (task1.Result, task2.Result, task3.Result, task4.Result, task5.Result, task6.Result);
        }

        protected async Task<(T1, T2, T3, T4, T5, T6, T7)> WhenAll<T1, T2, T3, T4, T5, T6, T7>(Task<T1> task1,
            Task<T2> task2, Task<T3> task3, Task<T4> task4, Task<T5> task5, Task<T6> task6, Task<T7> task7)
        {
        ...
        protected async Task<(T1, T2, T3, T4, T5, T6, T7, T8)> WhenAll<T1, T2, T3, T4, T5, T6, T7, T8>(Task<T1> task1,
            Task<T2> task2, Task<T3> task3, Task<T4> task4, Task<T5> task5, Task<T6> task6, Task<T7> task7,
            Task<T8> task8)
        {
        ...
        protected long GetCurrentTimestampMicrosec()
        {
            return _stopwatch == null || !_stopwatch.IsRunning
                ? -1
                : _stopwatch.ElapsedTicks / (TimeSpan.TicksPerMillisecond / 1000);
        }
        ...
        protected void InitializeCalculatorGraph()
        {
            ...
            var baseConfig = textConfig == null
                ? null
                : CalculatorGraphConfig.Parser.ParseFromTextFormat(textConfig.text);
            if (baseConfig == null)
                throw new InvalidOperationException(
                    "Failed to get the text config. Check if the config is set to GraphRunner");
            ConfigureCalculatorGraph(baseConfig);

            if (inferenceMode != InferenceMode.CPU) calculatorGraph.SetGpuResources(GpuManager.GpuResources);
        }
        ...
        protected void SetImageTransformationOptions(PacketMap sidePacket, ImageSource imageSource,
            bool expectedToBeMirrored = false)
        {
            ...
            Debug.Log(
                $"input_rotation = {inputRotation}, input_horizontally_flipped = {inputHorizontallyFlipped}, input_vertically_flipped = {inputVerticallyFlipped}");
        ...
        protected WaitForResult WaitForAsset(string assetName, string uniqueKey, long timeoutMillisec,
