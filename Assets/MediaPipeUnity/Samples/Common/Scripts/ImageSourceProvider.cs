...
namespace Mediapipe.Unity.Sample
{
    public static class ImageSourceProvider
    {
        ...
        public static ImageSourceType CurrentSourceType
        {
            get
            {
                if (ImageSource is WebCamSource) return ImageSourceType.WebCamera;
                if (ImageSource is StaticImageSource) return ImageSourceType.Image;
                if (ImageSource is VideoSource) return ImageSourceType.Video;
        ...
        public static void Switch(ImageSourceType imageSourceType)
        {
            switch (imageSourceType)
            {
                ...
                default:
                {
                    throw new ArgumentException($"Unsupported source type: {imageSourceType}");
                }
            }
        }
    }
